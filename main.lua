...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("PinatHub • Session: %s • %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "PinatHub • " .. os.date("%x %X") }
        }}
    }
            ...
            if url ~= "" and url:find("discord.com/api/webhooks/") then
                local payload = {
                    ["embeds"] = {{
                        ...
                        ["footer"] = { ["text"] = "PinatHub | Sailor Piece" }
                    }}
                }
                task.spawn(function()
                    pcall(function()
...
GB.Information.Right.Others:AddButton({ 
    ...
    Func = function()
        setclipboard("https://discord.gg/qhhSQQ332B")
    end
})
...
ThemeManager:SetFolder("PinatHub | Sailor Piece")
SaveManager:SetFolder("PinatHub/SailorPiece")
...
Library:Notify("PinatHub Script loaded.", 2)
Library:Notify("Report bug and give suggestion in our Discord!", 5)
